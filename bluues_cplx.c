...
{
pot1 = pot1 + srf_ses_mol_1.srf_pt[m].pot;
pot2 = pot2 + srf_ses_mol_2.srf_pt[h].pot;
nnp = nnp+1;
is_in_int_1[m] = 1;
is_in_int_2[h] = 1;
...
if(nnp > 0)
if(flag_par.verbose)
fprintf(fp2, "%5i %4s %4s %4i %c pot: %7.3lf -- %5i %4s %4s %4i %c pot:  %7.3lf -- d: %10.3lf -prod.: %9.3lf\n",
               sys.atoms[i].at_n,
               sys.atoms[i].at_name,
               sys.atoms[i].res_name,
               sys.atoms[i].res_n,
...
void check_cmd_line(int argc, char *argv[], struct Flag_par *flag_par)
{
	...
	printf("-pa x (area per point on SES in A^2, 0.05 default)\n"); 
	...
	printf("-g x (surface tension coefficient kJ/(A^2 mol), 0.1  default)\n"); 
...
fprintf(fpsum," --elec solv    energy: %14.4lf (kJ/mol)\n", sum.coul_solv_nrg + sum.born_nrg);
